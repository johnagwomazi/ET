...
function BrandLogo({ to = "/", className = "" }) {
  ...
  if (!to) return <div className={`inline-flex items-center gap-3 ${className}`}>{content}</div>;
  return <Link to={to} className={`inline-flex items-center gap-3 ${className}`}>{content}</Link>;
}
